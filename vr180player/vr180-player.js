...
function updateCanvasAspectRatio() {
  ...
  if (videoWidth > 0 && videoHeight > 0) {
	renderer.domElement.style.aspectRatio = `${videoWidth / videoHeight}`;
  }
}
...
function onWindowResize() 
{
	...
	if (renderer.xr && renderer.xr.isPresenting) return;
		...
		if (is2DMode) {
			...
			// Aspect ratio will be set dynamically based on video metadata
		} else {
		...
		if (is2DMode) updateCanvasAspectRatio();
	}
}
...
function start2DMode() {
	...
	// Update canvas aspect ratio based on video metadata
	updateCanvasAspectRatio();
